...
public class BeautyByteDanceImpl implements IBeautyByteDance {
    ...
    private ImageUtil mOutImageUtil;
    ...
    public int process(int texId, int texType, int width, int height, int rotation) {
        ...
        if (texType == GLES11Ext.GL_TEXTURE_EXTERNAL_OES) {
        ...
        return mOutImageUtil.transferTextureToTexture(dstTexture,
                BytedEffectConstants.TextureFormat.Texure2D,
                BytedEffectConstants.TextureFormat.Texure2D,
                width, height, transition);
    }
